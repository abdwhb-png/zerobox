...
NETWORK_DENIAL_DIAGNOSTICS_PATCH="$SCRIPT_DIR/upstream-network-denial-diagnostics.patch"
if [ -f "$NETWORK_DENIAL_DIAGNOSTICS_PATCH" ]; then
    echo "    network-denial-diagnostics"
    patch --fuzz=0 -p0 < "$NETWORK_DENIAL_DIAGNOSTICS_PATCH"
fi
